...
<template>
  <div>
    ...
    <!-- Dropdown -->
    <div
      class="hidden my-4 text-base list-none bg-white divide-y divide-gray-100 rounded-md shadow"
      id="language-dropdown-menu"
    >
      <ul class="h-80 overflow-y-auto py-2 font-medium" role="none">
        <li
          v-for="itemLocale in availableLocales"
          :key="'lang-' + itemLocale.code"
        >
          <NuxtLink
            @click.prevent.stop="setLocale(itemLocale.code)"
            :to="localePath(route.fullPath, itemLocale.code)"
            class="block px-3 py-2 text-sm text-gray-700 hover:bg-gray-100"
            role="menuitem"
          >
            <div
              class="flex items-center w-24"
              :class="{ 'cursor-not-allowed': itemLocale.selected }"
            >
              <div class="w-full">
